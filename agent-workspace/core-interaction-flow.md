...
- **空闲自动挂起**：不设显式过期时间，只记录最后一次操作时间；空闲超过阈值（默认 10 分钟）自动挂起并中断本次会话。挂起为**非终态**——患者再次询问时直接按复诊流程继续（以挂起会话为 `parentSessionId` 创建 follow_up）。另：大模型判断“本院不具备”处置条件时仍**转大医院（升级终止）**。
...
graph TD
    ...
    subgraph 全局并行 [横切·打断/升级 任意环节并行]
        ...
        J1["空闲挂起<br/>最后操作后约10分钟无操作"]:::interrupt
        J2((("转大医院 - 升级终止"))):::terminate
        J3["会话挂起<br/>非终止·可复诊继续"]:::interrupt
