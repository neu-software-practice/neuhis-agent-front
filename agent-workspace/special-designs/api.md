...
workbenchApi.ackAdvice(input)
workbenchApi.askLockedQuestion(input)
workbenchApi.classifyFollowUpIntent(input)
workbenchApi.streamConsultationReply(input, handlers)
workbenchApi.reportVitals(input)
...
> medAgent 处置只有 `MEDICATION` / `ADVICE_ONLY` / `REFERRAL`，**无院内自动化治疗执行**；需院内操作 / 手术的情形 medAgent 会给 `REFERRAL`。需求与 UI 文档保留「自动化治疗」作为产品分支，因此前端 contract 预留 `/treatment-execution`，由后端业务层或 mock 产出和推进；仅接 medAgent 三分类时，该分支不会出现。
...
- 首批 schema 清单以 `detailed-design.md` §6.5 为准，必须覆盖 patient / visits / workbench 全部 facade 方法、`TimelineItem`、`FlowCard` 和 `AssistantStreamEvent`。
...
- `workbenchApi.submitFulfillment`
- `workbenchApi.submitTreatmentExecution`
- `workbenchApi.ackAdvice`
- `workbenchApi.streamAssistantMessage`
...
- `LAB_PAYMENT_SUCCEEDED` / `PAYMENT_FAILED` / `PAYMENT_DEFERRED` / `MEDICATION_PAID`
- `DIAGNOSIS_READY`
- `TREATMENT_DECIDED` / `MEDICATION_FULFILLED` / `TREATMENT_SCHEDULED` / `TREATMENT_ARRIVED` / `TREATMENT_STARTED` / `TREATMENT_COMPLETED` / `ADVICE_ACKNOWLEDGED`
...
| `POST /visits/:id/payments`（药品费）+ `POST /visits/:id/fulfillment` | 后端 → `POST /sessions/{id}/purchase-result` | `DRUG_QUERY` 查规格对用户透明，`PURCHASE` 才是用户确认点 |
| `POST /visits/:id/treatment-execution` | （无直接对应） | 自动化治疗是后端业务层 / mock 能力，不推进 medAgent 主循环；完成后前端进入完成态 |
| `GET /visits/:id/diagnosis` / 诊断卡 | `Step.Result.diagnosis`（`DONE` 时） | 诊断随 `DONE` 的 `Result` 返回 |
| `POST /visits/:id/classify-intent` | 前端 / 后端业务层 | 完成态咨询/复诊意图分类，medAgent 无此能力，由独立 LLM 分类 |
| `POST /visits/:id/lock-question` / `consult` | 前端 / 后端业务层 | 锁定态疑问与完成态咨询为旁路 LLM 问答，不推进 medAgent 主循环 |
| `POST /visits/:id/vitals` | `POST /sessions/{id}/vitals` | 急症复检；命中后统一转成前端 `emergency` SSE / machine `EMERGENCY_DETECTED` |
...
- **无院内治疗执行（暂）**：medAgent 处置只有 `MEDICATION` / `ADVICE_ONLY` / `REFERRAL`，需院内操作/手术直接 `REFERRAL`。需求与 UI 文档保留"自动化治疗"分支，本期由前端 / mock 通过 `/visits/:id/treatment-execution` 演示，待后端业务层补充治疗执行能力后接入；在仅有三选一的 HTTP 后端下，治疗类情形按 `REFERRAL` 映射为终止卡 `reason: referral` 或 `capability_insufficient`。
